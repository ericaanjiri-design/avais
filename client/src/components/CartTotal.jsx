...
const CartTotal = ({method, setMethod}) => {
    ...
    const isOrderPage = location.pathname.includes('place-orders')
  return (
    <div>
      <h3 className='bold-22'>Order Summery <span className='bold-14 text-secondary'>({getCartCount()} Items)</span></h3>
          ...
          <div className='my-6'>
            ...
            <div className='flex gap-3'>
               <div
                ...
                className={`${method === "COD" ? "btn-secondary" : "btn-light"} !py-1 text-xs cursor-pointer`}
                >Cash on Delivery
                </div>
                <div
                onClick={() => setMethod('stripe')}
                className={`${method === "stripe" ? "btn-secondary" : "btn-light"} !py-1 text-xs cursor-pointer`}
                >Stripe</div>
            </div>
            </div>  
